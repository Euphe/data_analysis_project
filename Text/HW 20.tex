...
The obtained clusters are following (we visualize them by pairplots). They  don't match to MPAA\_Rating directly, but divide the data into reasonable descriptive categories. Sum of squared distances from points to cluster centers, k=5: 13.612553363194289:
...
We would specially distinguish cluster 3 (purple colored), the old films, which are mostly not long and with small amount of deaths. Cluster 4 (yellow colored) is not large and depicts the films with the highest number of deaths, they differ in length, but are all relatively the new ones. 
...
The code for bootstrap implication and the confidence intervals is below. We use pivotal and non-pivotal bootstrap for the confidence intervals. It can be seen from the graphs, that they are almost coincide, so the destributian of the feature is close to Gaussian.
...
\end{Verbatim}
    ...
    The results of the implication of principal components are the following.
